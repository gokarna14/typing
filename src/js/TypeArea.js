...
import '../css/TypeArea.css'

const TypeArea = () => {
    useEffect(() => {
        var today = new Date(), initialT = today.getHours()*60 + today.getMinutes() + today.getSeconds()/60;
        localStorage.setItem("initialT", initialT)
      }, []);
    ...
    const[index, setIndex] = useState(parseInt(Math.random() * toType.length))
    const[temp, setTemp] = useState(['a', 'b', 'c'])

    const setCrMis = (cr, mis)=>{
        var temp = new Date(),
            initialT =  localStorage.getItem("initialT"),   
            now = temp.getHours()*60 + temp.getMinutes() + temp.getSeconds()/60,
            r = (parseFloat(cr)/(parseFloat(cr)+parseFloat(mis))).toFixed(3),
            wpm = r*(parseFloat(cr)/5)/(now-initialT)
        
        console.log(now-initialT)
        console.log(now)
        localStorage.setItem("cr", cr)
        localStorage.setItem("mis", mis)
        localStorage.setItem("correct", JSON.stringify({name:'Correct', value:cr}))
        localStorage.setItem("mistakes", JSON.stringify({name:'Mistakes', value:mis}))
        localStorage.setItem("wpm", JSON.stringify({name:'Words per minute', value:wpm.toFixed(3)}))
        var ctm = mis === 0 ? cr : cr/mis
        localStorage.setItem("ctm", JSON.stringify({name:'Correct to mistake ratio', value:ctm.toFixed(3)}))

    }


    const update = (isCorrect)=>{
        var cr = localStorage.getItem("cr"), mis = localStorage.getItem("mis")
        if (isCorrect){
            setCrMis(parseInt(cr) + 1, mis)
        }
        else{
            setCrMis(cr, parseInt(mis)+1)
        }
    }
    setCrMis(0, 0)

    const typing = [...Array(typed.length).keys()].map(
        (e)=>{
            var cl = 'text-success', toDisplay = typed[e], isCorrect = true
            
            if (typed[e] !== toType[index][e])
            {
                cl = 'text-danger'
                if (toDisplay === ' '){
                    toDisplay = '_'
            ...
            return <span className={cl}>{toDisplay}</span>
        }
    )
    ...
    const result = [
        ...
        JSON.parse(localStorage.getItem("ctm")),
    ...
    return(
        <div>
            <div id='toType' class="border border-primary rounded">
                {
                    (
                        ()=>{
                            if (showPara){
                                return <h4>{toType[index]}</h4>
                            }
                        }
                    )()
                }
            ...
            <input type="text" placeholder="Type here..." value={typed} onChange={(e)=>{ setTyped(e.target.value) }} />
            <br />
            <div id='typedText'>
                <br />
               <h4>{typing}</h4>
